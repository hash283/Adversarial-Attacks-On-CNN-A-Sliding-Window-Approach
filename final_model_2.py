...
for eps in epsilon:
    ...
    for i in range(x_test.shape[0]):
        ...
        #plt.imshow(perturbations)
        #plt.show()
        window = sliding_window_3d(x_test[i],perturbations,[30,30],10,eps)
        #wn.append(window)
        wndw = tf.convert_to_tensor(window)
        #print(wndw.shape)
        #plt.figure(figsize=(1.5,1.5))
        ...
        window_out = n_model.predict(wndw)
        for j in range(window_out.shape[0]):
            if np.argmax(window_out[j]) == np.argmax(y_test[i]):
                curr += 1
            else:
                final_out.append(j)
        out.append(curr)
...
print("Final Counter of the windows : ")
print(Counter(final_out))
#     for tst in window:
